...
<footer class="flex-wrap md:justify-start md:flex-nowrap w-auto border rounded-lg text-md">
  <div class="py-6 px-6">
    <!-- Grid -->
    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
      <div class="flex flex-col col-span-full lg:col-span-1">
        <a class="footer-brand text-xl font-bold text-blue" id="brand" href="/" aria-label="Brand">Tomss14</a>
        <a class="footer-brand text-md font-semibold text-blue" id="slug" href="/" aria-label="Brand Slug">Free Games for You!</a>
        <a class="footer-logo" href="/">
          <img src="{{ asset('fav-icon-new.png') }}">
        </a>
      </div>
      <!-- End Col -->

      <div class="col-span-1">
        <h4 class="font-semibold text-blue">Games</h4>

        <div class="mt-3 grid space-y-3">
          <p><a class="footer-col inline-flex gap-x-2 text-secondary" href="/games?sortBy=recent">Recent Uploaded Games</a></p>
          <p><a class="footer-col inline-flex gap-x-2 text-secondary" href="/games/popular-games">Popular Games</a></p>
          <p><a class="footer-col inline-flex gap-x-2 text-secondary" href="/games">All Games</a></p>
        </div>
      </div>
      <!-- End Col -->

      <div class="col-span-1">
        <h4 class="font-semibold text-blue">Platform</h4>

        <div class="mt-3 grid space-y-3">
          <p><a class="footer-col inline-flex gap-x-2 text-secondary" href="/games/windows-games">Windows</a></p>
          <p><a class="footer-col inline-flex gap-x-2 text-secondary" href="/games/ps1-games">PlayStation 1</a></p>
          <p><a class="footer-col inline-flex gap-x-2 text-secondary" href="/games/ps2-games">PlayStation 2</a></p>
        </div>
      </div>
      <!-- End Col -->

      <div class="col-span-2">
        <h4 class="font-semibold text-blue">Request Games?</h4>
    
        <form id="req-game" action="{{ route('req-game') }}" method="POST">
          @csrf
          <div class="mt-4 flex flex-col justify-between items-center gap-2 sm:flex-row sm:gap-3 rounded-lg py-2">
            <div class="flex flex-col gap-2" style="width: 100%">
              <div class="w-full">
                  <!-- Input untuk Nama Game -->
                  <input type="text" id="game-name" name="game_name" class="py-3 px-4 block w-full border-transparent rounded-lg text-blue text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none" placeholder="Enter game name" required>
              </div>
  
              <div class="w-full">
                  <!-- Dropdown untuk Platform -->
                  <select id="platform-id" name="platform_id" class="py-3 px-4 block w-full border-transparent rounded-lg text-blue text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none" required>
            ...
            <!-- Tombol Submit -->
            <button type="submit" class="w-full sm:w-auto whitespace-nowrap p-3 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-secondary hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                Submit
            </button>
          </div>
        </form>
        <div id="notification" class="hidden mt-4 p-4 text-green-600 bg-green-100 rounded-lg"></div>
      </div>
    
      <script>
        document.getElementById('req-game').addEventListener('submit', async function (event) {
          ...
          try {
              ...
              if (response.ok) {
                  ...
                  document.getElementById('notification').textContent = 'Request game submitted';
